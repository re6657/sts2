...
/// <summary>
/// Keeps a bundle request single-shot and ages its fallback from the request,
/// not from the overlay lifetime. Each tick produces at most one input.
/// </summary>
public sealed class BundleSelectionRequestGate
{
    ...
    public BundleSelectionInput Tick(bool hasValidHitbox, bool timeoutRecoveryRequested)
    {
        if (!Attempted)
        {
            Attempted = true;
        ...
        if (!FallbackSent && hasValidHitbox &&
            (RequestAge >= HitboxFallbackFrame || timeoutRecoveryRequested))
        {
            FallbackSent = true;
