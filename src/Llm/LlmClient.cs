...
public class LlmClient
{
    ...
    public LlmClient(LlmConfig config)
    {
        ...
        _consoleEnabled = MainFile.ConsoleEnabled;
        _characterPrompt = LoadCharacterPrompt(config.Character);

        // Set up files in llm_data subdirectory to keep mod root clean
        var asmDir = Path.GetDirectoryName(
            System.Reflection.Assembly.GetExecutingAssembly().Location) ?? ".";
        var dataDir = Path.Combine(asmDir, "llm_data");
        Directory.CreateDirectory(dataDir);
        var ts = DateTime.Now.ToString("yyyyMMdd_HHmmss");
        _logPath = Path.Combine(dataDir, $"llm_log_{ts}.txt");
        _memoryPath = Path.Combine(dataDir, $"llm_memory_{ts}.md");
        _historyPath = Path.Combine(dataDir, $"llm_history_{ts}.json");
        ...
        WriteConsole($"║  TokenSpire2 LLM Session Started         ║", ConsoleColor.Cyan);
        WriteConsole($"║  Model: {_model,-33} ║", ConsoleColor.Cyan);
        if (!string.IsNullOrEmpty(_characterPrompt))
            WriteConsole($"║  Character: {config.Character,-28} ║", ConsoleColor.Cyan);
        WriteConsole($"╚══════════════════════════════════════════╝", ConsoleColor.Cyan);
    }
    ...
    public async Task<string> SendAsync(string userMessage, string? context = null)
    {
        ...
        // Console output — show a summary of what's being sent to LLM
        var contextLabel = context switch
        {
            ...
            "restsite" => "REST SITE",
            "shop" => "SHOP",
            _ when context?.StartsWith("overlay:") == true => context.Substring(8).ToUpper(),
            "gameover_reflection" => "GAME OVER REFLECTION",
            _ => context ?? "?"
        };
        WriteConsole($"\n┌─ Turn {_history.Count / 2} [{contextLabel}] ({DateTime.Now:HH:mm:ss}) ─", ConsoleColor.Yellow);
        WriteConsole($"│ [USER — sending to LLM]", ConsoleColor.Yellow);
        // Print user message with truncation for console readability
        foreach (var line in userMessage.Split('\n'))
        {
            var display = line.Length > 120 ? line.Substring(0, 117) + "..." : line;
            WriteConsole($"│ {display}", ConsoleColor.Gray);
        }
        WriteConsole($"└{'─' * 40}", ConsoleColor.Yellow);
        ...
        // Console output — show the assistant response
        if (!string.IsNullOrEmpty(thinking))
        {
            WriteConsole("--- THINKING ---", ConsoleColor.DarkGray);
            WriteConsole(TruncateForConsole(thinking), ConsoleColor.DarkGray);
        }
        WriteConsole("--- ASSISTANT ---", ConsoleColor.Green);
        WriteConsole(TruncateForConsole(result), ConsoleColor.White);
    ...
    private static string LoadCharacterPrompt(string character)
    {
        try
        {
            var dir = Path.GetDirectoryName(
                System.Reflection.Assembly.GetExecutingAssembly().Location) ?? ".";
            var path = Path.Combine(dir, "character_prompts", $"{character}.md");
            if (!File.Exists(path)) return "";
            var text = File.ReadAllText(path).Trim();
            MainFile.Logger.Info($"[AutoSlay/LLM] Loaded character prompt for {character} ({text.Length} chars)");
            return text;
        }
        catch { return ""; }
    }
    ...
    private void WriteConsole(string text, ConsoleColor color)
    {
    ...
    private static string TruncateForConsole(string text, int maxLines = 40, int maxLen = 150)
    {
        ...
        {
            if (count >= maxLines) { result.AppendLine("... [truncated]"); break; }
            result.AppendLine(line.Length > maxLen ? line.Substring(0, maxLen - 3) + "..." : line);
            count++;
        }
